...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	public static IEvent createRepeatableEvent(EventDataPasser event) {
		Element el = new Element("event");
		...
		el.addAttribute(new Attribute("repeat-type", String.valueOf(event.getType())));
		el.addAttribute(new Attribute("id", Util.generateId()));
		el.addAttribute(new Attribute("hour", String.valueOf(event.getHh())));
		el.addAttribute(new Attribute("min", String.valueOf(event.getMm())));
		el.addAttribute(new Attribute("startDate", event.getStartDate().toString()));
		if (event.getEndDate() != null)
			el.addAttribute(new Attribute("endDate", event.getEndDate().toString()));
		el.addAttribute(new Attribute("period", String.valueOf(event.getPeriod())));
		// new attribute for wrkin days - ivanrise
		el.addAttribute(new Attribute("workingDays",String.valueOf(event.isWorkDays())));
		el.appendChild(event.getText());
		rep.appendChild(el);
		return new EventImpl(el);
	}
	...
	public static IEvent getEvent(CalendarDate date, int hh, int mm) {
		Day d = getDay(date);
		//SER316 Task 1 Complexity Step3
		//if (d == null)
		//	return null;
		Elements els = d.getElement().getChildElements("event");
		for (int i = 0; i < els.size(); i++) {
